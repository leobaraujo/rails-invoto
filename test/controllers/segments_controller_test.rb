...
class SegmentsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create segment" do
    assert_difference("Segment.count") do
      post segments_url, params: { segment: { name: @segment.name, region_id: @segment.region_id, status: @segment.status } }
    end
  ...
  test "should update segment" do
    patch segment_url(@segment), params: { segment: { name: @segment.name, region_id: @segment.region_id, status: @segment.status } }
